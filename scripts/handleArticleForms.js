...
function processArtEditFrmData(event, articleId, offset, totalCount, serverUrl) {
    ...
    const articleData = {
        ...
        imageLink: document.getElementById("imageUrl").value.trim(),
...
function addArticle(event, serverUrl) {
    ...
    fetch(`${serverUrl}/article`, postReqSettings)
        ...
        .catch(error => {
            console.log(error);
            window.alert(`Failed to save the updated article on server. ${error}`);
        }).finally(() => window.location.hash = `#article/${id}`);

}

async function addComment(evt, url, id) {
    event.preventDefault()

    const formData = {
        text: document.getElementById("content").value.trim(),
        author: document.getElementById("author").value.trim()
    };

    if (!(formData.author && formData.text)) {
        window.alert("Please, enter article author and content" + JSON.stringify(formData));
        return;
    }
    ...
    console.log(`${url}/api/article/${id}/comment`)

    try {
        const response = await fetch(`${url}/article/${id}/comment`, postReqSettings);
        if (!response.ok) {
            throw new Error(`Server answered with ${response.status}: ${response.statusText}.`);
        }
    } catch (error) {
        window.alert(`Failed to save the updated comment on server. ${error}`);
    } finally {
